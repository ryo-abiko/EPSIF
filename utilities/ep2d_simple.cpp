...
void ep2d_simple_gray(double *output, double *input, const int M, const int N, const int w_size, double threshold, const int iter_max){
    ...
    temporary_img_yoko = (double *)mxCalloc((M+2*ws)*(N+2*ws),sizeof(double));
    temporary_img_tate = (double *)mxCalloc((M+2*ws)*(N+2*ws),sizeof(double));
    input_img = (double *)mxCalloc((M+2*ws)*(N+2*ws),sizeof(double));
...
void ep2d_simple_color(double *output, double *input, const int M, const int N, const int w_size, double threshold, const int iter_max){
...
void mexFunction( int Nreturned, mxArray *returned[], int Noperand, const mxArray *operand[] ){
    ...
    // 処理を実行
    if (input_size[2] == 1){
        ep2d_simple_gray(output,input,input_size[0],input_size[1],(int) *mxGetPr(operand[1]),*mxGetPr(operand[2]),(int) *mxGetPr(operand[3]));
    }else{
        ep2d_simple_color(output,input,input_size[0],input_size[1],(int) *mxGetPr(operand[1]),*mxGetPr(operand[2]),(int) *mxGetPr(operand[3]));
    }
}
